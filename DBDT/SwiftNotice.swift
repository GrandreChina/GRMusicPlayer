...
//        label.center = CGPointMake(mainView.center.x, mainView.center.y - 30)
        
        window.windowLevel = UIWindowLevelAlert
        
//        window.center = getRealCenter()
//        GR---重新定义了显示位置
        window.center = CGPointMake(getRealCenter().x, getRealCenter().y - 150)
        ...
        if autoClear {
            ...
            self.performSelector(selector, withObject: window, afterDelay: NSTimeInterval(time))
        }
    }
...
//    ----GR----
    func noticeOnlyTextAutoClearGR(text:String){
        SwiftNotice.showTextAutoGR(text)
    }
...
//        if autoClear {
//            let selector = Selector("hideNotice:")
//            self.performSelector(selector, withObject: window, afterDelay: NSTimeInterval(1))
//        }
    }
